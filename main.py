...
from sgm.util import (exists, instantiate_from_config, isheatmap, print0, seed_anything,
                      check_nvlink_connectivity, report_gpu_memory)

MULTINODE_HACKS = True

def print_model_structure(model, max_depth=3):
    """打印模型结构，找出实际的模块名称"""
    print0(f"[bold cyan]\[DEBUG][Model Structure][/bold cyan]")
    ...
    def print_modules(module, prefix="", depth=0):
        ...
        for name, child in module.named_children():
            ...
            if isinstance(child, torch.nn.Linear):
                print0(f"  {'  ' * depth}📍 {full_name} (Linear: {child.in_features} -> {child.out_features})")
            else:
                print0(f"  {'  ' * depth}📁 {full_name} ({type(child).__name__})")
...
def print_transformer_block_details(model):
    """查看 TransformerBlock 的详细结构"""
    print0(f"[bold cyan]\[DEBUG][TransformerBlock Details][/bold cyan]")

    # 查看第一个 TransformerBlock 的结构
    first_block = model.model.blocks[0]

    def print_modules_recursive(module, prefix="", max_depth=5, current_depth=0):
        ...
        for name, child in module.named_children():
            ...
            if isinstance(child, torch.nn.Linear):
                print0(f"  {'  ' * current_depth}📍 {full_name} (Linear: {child.in_features} -> {child.out_features})")
            else:
                print0(f"  {'  ' * current_depth}📁 {full_name} ({type(child).__name__})")
                print_modules_recursive(child, full_name, max_depth, current_depth + 1)

    print_modules_recursive(first_block, "blocks.0")

def generate_target_modules(model, num_blocks=16):
    """自动生成 target_modules 列表"""
    target_modules = []

    # Self-Attention 和 FeedForward 模块
    for i in range(num_blocks):
        # Self-Attention
        target_modules.extend([
            f"blocks.{i}.attention.wqkv",
            f"blocks.{i}.attention.wo",
        ])

        # FeedForward
        target_modules.extend([
            f"blocks.{i}.feed_forward.w1",
            f"blocks.{i}.feed_forward.w2",
            f"blocks.{i}.feed_forward.w3",
        ])

    # Final layer
    target_modules.append("final_layer.linear")

    return target_modules

def freeze_non_cross_attention_params(model, trainable_keywords=None):
    """冻结除了 cross_attention 和 LoRA 相关的所有参数"""
    if trainable_keywords is None:
        trainable_keywords = [
            'cross_attention',
            'cross_attention_norm',
            'adaLN_modulation_cross',
            'pose_embedder',
            'timestamp_embedder',
            'pose_cond_mlp',
            # 'lora_'  # 添加 LoRA 参数
    ...
    print0(f"[bold red]\[main][freeze_params][/bold red] Freezing parameters...")
    print0(f"[bold red]\[main][freeze_params][/bold red] Total parameters: {total_params:,}")
    print0(f"[bold red]\[main][freeze_params][/bold red] Trainable parameters: {trainable_params:,}")
    print0(f"[bold red]\[main][freeze_params][/bold red] Frozen parameters: {total_params - trainable_params:,}")
    print0(f"[bold red]\[main][freeze_params][/bold red] Trainable ratio: {trainable_params/total_params:.2%}")

    print0(f"[bold red]\[main][freeze_params][/bold red] Trainable parameter names:")
...
def apply_peft_lora(model, config):
    if hasattr(config.model, 'params') and config.model.params.get('use_lora', False):
        from peft import LoraConfig, get_peft_model, TaskType

        lora_config_params = config.model.params.get('lora_config', {})

        target_modules = generate_target_modules(model, num_blocks=16)
        ...
        print0(f"[bold green]\[main][LoRA][/bold green] Applying LoRA to {len(target_modules)} modules")
        print0(f"First few targets: {target_modules[:5]}")
        print0(f"Last few targets: {target_modules[-5:]}")
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,  # 23
        help="seed for seed_everything (default: -1, i.e., random seed)",
    )
    parser.add_argument(
        "--seed_rank",
        type=str2bool,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="W&B API key; prefer the WANDB_API_KEY environment variable",
    )
    parser.add_argument(
        "--wandb_project",
        type=str,
    ...
    parser.add_argument(
        ...
        const=True,
        default=False,
        help="enable multi-node distributed training setup",
    )
...
class SetupCallback(Callback):
    ...
    def on_fit_start(self, trainer, pl_module):
        ...
        report_gpu_memory(rank=trainer.local_rank, prefix="main][SetupCallback")
...
def init_wandb(save_dir, opt, config, group_name, name_str):
    ...
    if not wandb.api.api_key:
        wandb.login(key=opt.wandb_key) if opt.wandb_key else wandb.login()
    if opt.debug:
...
def setup_distributed_env():
    ...
    tcp_port = os.environ.get('LUBAN_AVAILBLE_PORT_0', '12355')
    os.environ['MASTER_ADDR'] = master_addr
    os.environ['MASTER_PORT'] = tcp_port

    print(f'Distributed setup: rank={rank}, world_size={world_size}, master={master_addr}:{tcp_port}')
...
if __name__ == "__main__":
    ...
    if opt.resume:
        ...
        if os.path.exists(opt.logdir):
            auto_resumed = False
            ckpt_files = []
            #print(f"[bold red]\[main][/bold red] logdir: {opt.logdir}")
            for sub_dir in sorted(os.listdir(opt.logdir)):
                #print(f'sub_dir: {sub_dir}')
                #print(f'name + opt.postfix: {name + opt.postfix}')
                #print(f'opt.logdir: {opt.logdir}')
                ...
                if sub_dir.endswith(name + opt.postfix):
                    ...
                    if opt.checkpoint_auto_resume and not opt.debug:
                        ...
                        print0(f"[bold red]\[main][/bold red] Found checkpoint files: {ckpt_files}")
                       #print(None.shape)
                        if ckpt_files:
                            # ckpt = ckpt_files[0]
                            ckpt = None
                            for ckpt_file in ckpt_files:
                                if ckpt_file is not None and os.path.isfile(ckpt_file):
                                    ckpt_filesize_kb = os.path.getsize(ckpt_file)/1024.
                                    if ckpt_filesize_kb > 10:
                                        ckpt = ckpt_file
                                        break
                                    print0(f"[bold red]\[main][/bold red] Skip small checkpoint file: {ckpt_file} with size {ckpt_filesize_kb} KB")
                        else:
            ...
            if auto_resumed:
                ...
                if len(seed_str) > 0 and opt.seed != -1:
    ...
    if not opt.seed_rank:
        if opt.seed == -1:
            opt.seed = random.randint(0, 1000000)  # to make sure random data loading during resume
    ...
    try:
        ...
        # print_transformer_block_details(model)
        if hasattr(config.model, 'params') and config.model.params.get('use_lora', False):
            model = apply_peft_lora(model, config)
        if hasattr(config.model, 'params') and config.model.params.get('freeze_non_cross_attention', False):
            trainable_keywords = config.model.params.get('trainable_keywords', None)
            model = freeze_non_cross_attention_params(model, trainable_keywords)
        ...
        default_modelckpt_cfg = {
            ...
            "params": {
                "dirpath": ckptdir,
                "filename": "{epoch:04d}-{step:010}",  # "epoch={epoch:06}-step={step:07}" step same as global_step
                "verbose": True,
                "save_last": True,
            ...
            default_metrics_over_trainsteps_ckpt_dict = {
                "metrics_over_trainsteps_checkpoint": {
                    ...
                    "params": {
                        ...
                        "filename": "{epoch:04d}-{step:010}",  # "{epoch:06}-{step:09}"
                        "verbose": True,
                        "save_top_k": -1,
        ...
        # data
        if ((not opt.train) or opt.debug) and hasattr(config.data.params, "validation") and hasattr(config.data.params.train, "target"):
        ...
        if "batch_size" in config.data.params:
            ...
            bs, base_lr = (
                config.data.params.train.params.batch_size,
        ...
        if opt.train:
            try:
                ...
                # if trainer.global_rank == 0:
                #     model.model.save_pretrained('../output_peft/peft_lora')
