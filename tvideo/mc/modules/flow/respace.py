...
def compute_density_for_timestep_sampling(
    weighting_scheme: str, size: tuple, logit_mean: float = None, logit_std: float = None, mode_scale: float = None
):
    ...
    if weighting_scheme == "logit_normal":
        ...
        u = th.normal(mean=logit_mean, std=logit_std, size=size, device="cpu")
        u = th.nn.functional.sigmoid(u)
    elif weighting_scheme == "mode":
        u = th.rand(size=size, device="cpu")
        u = 1 - u - mode_scale * (th.cos(math.pi * u / 2) ** 2 - 1 + u)
    elif weighting_scheme == "logit_normal_trigflow":
        sigma = th.randn(size=size, device="cpu")
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def training_losses_32(self, model, *args, **kwargs):  # pylint: disable=signature-differs
        return super().training_losses_32(self._wrap_model(model), *args, **kwargs)
