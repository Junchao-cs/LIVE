...
class GaussianDiffusion:
    ...
    def set_seed_generator(self, generator):
        self.seed_generator = generator
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        """
        if noise is None:
            noise = th.randn_like(x_start) if self.seed_generator is None else th.empty_like(x_start).normal_(generator=self.seed_generator)
    ...
    def p_sample(
        ...
        noise = th.randn_like(x) if self.seed_generator is None else th.empty_like(x).normal_(generator=self.seed_generator)
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            img = th.randn(*shape, device=device) if self.seed_generator is None else th.randn(*shape, generator=self.seed_generator, device=device)
        ...
        if progress:
            ...
            indices = tqdm(indices, desc="[GaussianDiffusion] Sampling")
    ...
    def training_losses(self, model, x_start, timestep, model_kwargs=None, noise=None, skip_noise=False):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            #rint(f'output shape: {model_output.shape}')
            #rint(f'loss_type: {self.loss_type}, mean_type: {self.model_mean_type}, var_type: {self.model_var_type}')
            ...
            if model_kwargs.get("mask_ratio", False) and model_kwargs["mask_ratio"] > 0:
                ...
                terms["every frame"] = loss
                #terms["x_start_pred"] = x_start_pred
    ...
    def training_losses_32(self, model, x_start, timestep, model_kwargs=None, noise=None, skip_noise=False):
        """
        Compute training losses for a single timestep.
        :param model: the model to evaluate loss on.
        :param x_start: the [N x C x ...] tensor of inputs.
