...
from sgm.util import exists, isheatmap, print0
...
class ImageVideoLogger(Callback):
    ...
    def log_vid_local(
        ...
        for k in videos:
            ...
            if isinstance(videos[k], torch.Tensor) and videos[k].ndim == 5:
                ...
                frames = [videos[k][:, i] for i in range(videos[k].shape[1])]
                ...
                frames = (frames.numpy() * 255).astype(np.uint8)  # frames: (t, c, h, w)

                os.makedirs(root, exist_ok=True)
                # filename_gif = "{}_gs-{:06}_e-{:06}_b-{:06}.gif".format(
                #     k, global_step, current_epoch, batch_idx
                # )
                # path_gif = os.path.join(root, filename_gif)
                # save_numpy_as_gif(frames, path_gif, duration=1 / self.video_fps)
                filename_mp4 = "{}_gs-{:06}_e-{:06}_b-{:06}.mp4".format(
                    k, global_step, current_epoch, batch_idx
                )
                path_mp4 = os.path.join(root, filename_mp4)
                save_numpy_as_mp4(frames, path_mp4, fps=self.video_fps)
                # if exists(pl_module):
                #     assert isinstance(
                #         pl_module.logger, WandbLogger
                #     ), "log_videos only supports WandbLogger currently"
                #     wandb.log({f"{split}/{k}": wandb.Video(frames, fps=self.video_fps)})  # k is str
                #     # wandb.log({f"{split}/{k}": wandb.Video(frames, caption='caption', fps=self.video_fps)}, step=global_step)
    ...
    def check_frequency(self, check_idx):
        if ((check_idx % self.batch_freq) == 0 or (check_idx in self.log_steps)) and (
            check_idx > 0 or self.log_first_step  # seems starting from 1
        ):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        # global_step starts from 1, batch_idx starts from 0
        # not log after first step
        if not self.disabled and (pl_module.global_step > 2):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        ...
        if not self.disabled and (pl_module.global_step == 0 and self.log_before_first_step):
            self.log_img(pl_module, batch, batch_idx, split="train")
            self.log_vid(pl_module, batch, batch_idx, split="train")
...
def save_numpy_as_mp4(frames, path, fps):
    """
    save numpy array as mp4 file
    """
    # image_list = []
    # for frame in frames:
    #     image = frame.transpose(1, 2, 0)
    #     image_list.append(image)
    # imageio.mimsave(path, image_list, format="FFMPEG", fps=fps)
