...
def trace_handler(prof: torch.profiler.profile):
   ...
   if not os.path.exists(f"logs"):
       os.makedirs(f"logs")
   prof.export_memory_timeline(f"logs/{file_prefix}.html", device="cuda:0")


def profile_module(module_func: callable, module_input_shape: tuple, device: str = "cuda"):
    with torch.profiler.profile(
        ...
        for _ in range(5):
            ...
            with record_function("## tokenizer ##"):
                module_func(torch.randn(module_input_shape, device=device))
    if wandb.run is not None:
        html_path = glob.glob("logs/*.html")[-1]
        wandb.log({"memory_trace": wandb.Html(html_path, inject=False)})
    # memory cleanup
    gc.collect()
...
class CameraPose:
    ...
    def normalize_by_last(self) -> None:
        """
        Normalizes the camera poses by the first camera, i.e. computes the relative poses w.r.t. the first camera.
        After normalization, the first camera will have identity rotation and zero translation, i.e. the first camera = world coordinates.
        """
